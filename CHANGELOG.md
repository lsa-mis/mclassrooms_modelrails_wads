...
- **Notifications jobs + hardening (PR-5 of 5, completes v1).** Closes the v1 notifications stack with the digest channel and the retention sweep. `DigestMailerJob` runs every 15 minutes against an indexed range scan on `user_preferences.digest_next_due_at` (single SELECT regardless of user count, no per-user polling); for each due user it skips if DND or `digest_enabled?` is off, scopes to unseen notifications in `DIGEST_ELIGIBLE_CATEGORIES` since the last digest, and on enqueue bulk-stamps `seen_at` on every included notification — the load-bearing dedupe so consecutive digests never repeat the same item. `NotificationCleanupJob` runs daily at 3am with per-user batched deletion (chunks of 100 to release the SQLite write lock between rounds), honoring `retention_days` with a 2-day grace window and a `RETENTION_FLOORS["security"]` 1-year floor that protects security-category notifications regardless of user preference. Polished digest mailer template groups notifications by category (reuses `notifications.preferences.categories.*` from PR-4 for label vocabulary), renders each with a "View" link to the notifier's `#url`, and footer-links to `/account/notification_preferences` for cadence/pause/retention controls — both message and url wrapped in `render_safe_or_placeholder` so deleted notifiables degrade gracefully instead of breaking the digest. Hardening test file (`spec/requests/notifications_hardening_spec.rb`) covers the panel-flagged contracts the rest of the stack relies on: idempotent retry under the DB unique constraint (returns `:deduplicated` sentinel, no exception escapes), fan-out atomicity (transaction rolls back Event row when `notifications.insert_all!` raises), recipient deleted before digest job (`User#dependent: :destroy` cascades, scope finds nothing, no raise), notifiable deleted during render (placeholder string returned), throttle fail-open under cache miss (security mail still sends when `Rails.cache.increment` returns nil), concurrent mark-all-read with mid-batch arrival (late arrivals stay unread). Brakeman + Rubocop sweep clean. Test count climbs from PR-4's 1548 to 1582 (+34 examples for jobs, mailer, and hardening); 0 failures, 2 pending (the existing deferred Rouge tokens).

- **Notification preferences UI (PR-4 of 5).** New surface at `/account/notification_preferences` (linked from the user menu) lets users tune what notifications reach them and how. Four sections, each its own `<form>` with Turbo Stream auto-save (no Submit button — every toggle commits as it changes): a master "Pause non-essential notifications" DND toggle (Security alerts always go through, per `NotificationPreferences#allow?`'s security bypass); a 5×3 category × channel matrix (`security` / `account_access` / `workspace_activity` / `project_activity` / `billing` × `in_app` / `email` / `digest`) where Security's in-app + email cells render disabled with "Always on" help text and submit a hidden `true` so the JSONB carries the canonical state; digest cadence (Daily / Weekly) + time-of-day picker, displayed in the user's IANA timezone (stored on `UserPreferences.timezone`); and a retention dropdown (30 / 60 / 90 / 180 / 365 / Never) gated server-side by an allowlist (other values 422). Cadence or hour changes trigger an atomic recompute of `digest_next_due_at` so the indexed range scan PR-5's `DigestMailerJob` will rely on stays accurate. Bell-trigger gains a `title=""` tooltip ("X unread (Y hidden by Do Not Disturb)" via the existing `notifications.bell.unread_with_dnd` locale) when DND is on AND user has unread, surfacing the suppression to sighted hover/focus users without taking up nav-bar real estate. New tiny Stimulus controller `auto_submit` (~10 lines) wraps the auto-save pattern; new shared `_toggle.html.erb` partial used 16× on this one page (master + matrix). Plan-deviation notes: `Current.user.preferences` (not `user_preferences` per plan), `UserPreferences.timezone` (not `User.timezone` per plan), nav link added to user menu (plan called for `app/views/account/_navigation.html.erb` which doesn't exist — same recurring gotcha as PR-3 Task 12).
