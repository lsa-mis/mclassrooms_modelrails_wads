---
title: Signing In
description: How to sign in and out of MClassrooms with U-M single sign-on (Google or Okta)
keywords: sign in sign out sso single sign-on google okta weblogin u-m umich authentication account
---

# Signing In

MClassrooms uses **U-M single sign-on**. You sign in with the same University
of Michigan account you already use everywhere else — there is no separate
MClassrooms password to create or remember.
...
The **first** time you sign in, MClassrooms creates your account automatically
...
you into the same MClassrooms account.
...
directory are additional abilities granted by a MClassrooms administrator — see
...
MClassrooms is single-sign-on first, so the email/magic-link and passkey options
