...
# New-device sign-in alerts ("We noticed a new sign-in from ..."):
# - new_device_notification: email/notify the user the first time they sign in
#   from an unrecognized browser/OS. Fingerprints are still RECORDED when this
#   is off — the flag gates only the alert, so flipping it back on later keeps
#   full device history. Off for internal tools where every teammate's laptop
#   would fire one on day one.
